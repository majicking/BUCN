    ...
    function WithdrawCtrl($rootScope, $sessionStorage, $scope, $ionicLoading, $ionicNativeTransitions, AccountService, $stateParams) {
        ...
        function confirm(){
            if (!vm.model.withdrawType) {
                $scope.$emit('alertWarning', '此用户不能兑换，详情请联系管理员');
            }else if(vm.input.subAccountTypeOp == null
                || $.trim(vm.input.subAccountTypeOp) == ''){
                $scope.$emit('alertWarning', '请选择兑换账户');
            }else if(vm.model.amount < vm.input.money){
                $scope.$emit('alertWarning', '兑换额度超限');
            }else if(vm.model.withdrawLimit != null
                && vm.input.money < vm.model.withdrawLimit){
                $scope.$emit('alertWarning', '兑换额度低于最低额');
            }else if(vm.input.money%100 != 0){
                $scope.$emit('alertWarning', '兑换额度必须是100的整数倍');
